...
13. **Gestures the glass has no word for.** *(Interaction Grammar)* Touch
    ...
    copies context, toggles a column's tabs, unjoins the panel, and closes
    it. Opening a link un-joined still has no touch equivalent of its own: a
    link on glass always joins, and the header menu's unjoin is the way to
    the same place after the fact. A long press on a link is one possible
    home.

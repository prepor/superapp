...
```sh
./android.sh              # build, install, start, and follow the app's log
...
One command for four steps, none of them guessable. It builds `cargo-makepad`
from the exact Makepad revision the root `Cargo.toml` pins — the one on
`$PATH` belongs to whatever project installed it last, and the activity's Java
is compiled from the tool's own copy, so the wrong tool quietly produces an
app that behaves differently on the glass. It stages TDLib into the Cargo
output directory, which is where Makepad packages shared libraries from. It
builds with the SDK path, the package name and the label the phone knows this
app by, and the app's own `app/resources/android/AndroidManifest.xml.template`
rather than the tool's default. Then it installs and starts.
...
`lib/libtdjson.so` for `arm64-v8a`, or leave Telegram out with `--no-tdlib`.

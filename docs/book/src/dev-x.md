...
clicking it. `accel "copy" c` checks the accelerator actually drawn on the
control; `accel "copy" -` checks that it has no highlighted letter. Both wait
for current hit areas and also work under `--no-draw`.

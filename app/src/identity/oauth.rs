//! Shared Google OAuth for Mail and Calendar.
//!
...
//! developer supplies a Google Desktop-app registration through
...
/// `Debug` redacts, on the same rule as the protocol credentials.
...
/// What a completed sign-in establishes: which address consented, and the
/// grant to keep. The hour-long access token is not among them — the first
/// pass that needs one mints it off the refresh token
/// (the shared token cache), and one carried here would be
/// stale by the time an account is added.
#[derive(Clone)]
pub struct Signed {
    pub email: String,
    pub subject: String,
    pub scopes: String,
    /// The durable half — this goes to the keychain, and the hour-long
    /// access token is minted off it on the first pass that needs one.
    pub refresh: String,
}
...
impl Flow {
    ...
    pub async fn wait(self) -> Result<Signed, String> {
        ...
        for required in self
            .scopes
            .split_whitespace()
            .filter(|s| s.starts_with("https://") && !s.ends_with("userinfo.email"))
        {
            if !granted.split_whitespace().any(|s| s == required) {
                return Err(format!("Google did not grant {required}. Enable this scope in the consent screen and reconnect; the existing account is unchanged."));
            }
        }
        ...
        let claims = r
            ...
            .and_then(|v| serde_json::from_slice::<Identity>(&v).ok())
            .filter(|v| v.email_verified && !v.sub.is_empty())
            .ok_or("Google returned no verified identity")?;
        Ok(Signed {
            email,
            refresh,
            subject: claims.sub,

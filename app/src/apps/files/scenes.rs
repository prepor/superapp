...
fn file_card() -> Scene<Setup> {
    Scene::new("file card", (520.0, 420.0))
        ...
        .note("Text reads up to 64 KiB, images up to 20 MiB, and PDFs up to 64 MiB. The shared viewer measures the content and fits images and pages without cropping.")
        ...
        .node("clip", panel_fake(|_| Card::id("~/Downloads/2026/talk.mp4"), ""))
        .sized((520.0, 520.0))
        .about("a clip is the kit's surface at the card's width, the strip beneath, and play on the bar; the demo tree has no bytes behind the name, so nothing runs")
        .node("sound", panel_fake(|_| Card::id("~/Downloads/2026/episode.mp3"), ""))
        .sized((520.0, 260.0))
        .about("a sound is the strip alone")

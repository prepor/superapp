...
impl sync::Fetch for RecordingFetch {
    fn get(&mut self, request: &sync::Request) -> Result<sync::Response, String> {
...
fn hundreds_of_feeds_share_one_worker_and_each_refreshes_once() {
    ...
    assert_eq!(workers.len(), 1, "each worker owns a thread and database reader");
    ...
    assert_eq!(s.store().conn().query_row(
        "SELECT COUNT(*) FROM rss_feed WHERE requested != completed", [], |r| r.get::<_, i64>(0),
    ).unwrap(), 0);
    assert_eq!(s.store().conn().query_row(
        "SELECT error FROM rss_feed WHERE id=100", [], |r| r.get::<_, String>(0),
    ).unwrap(), "feed unavailable");

    // The retained worker discovers new subscriptions and skips removed ones.
    calls.borrow_mut().clear();
    s.store().write(|c| c.execute_batch(
    ...
    assert_eq!(RSS.workers(s.store()).iter().map(|worker| worker.name()).collect::<Vec<_>>(), names);
    for worker in &mut workers {
        worker.pass(s.world());
    }
...
fn manual_refresh_precedes_sleeping_feeds_and_periodic_deadlines_are_preserved() {
    ...
    let mut workers = RSS.workers(s.store());
    for worker in &mut workers {
        assert_eq!(worker.pass(s.world()), Wake::After(Duration::from_secs(900)));
    }
    assert!(calls.borrow().is_empty());

    env.clock.advance(30.0);
    s.store().write(|c| c.execute(
        "UPDATE rss_feed SET requested=requested+1 WHERE id=5", [],
    )).unwrap();
    for worker in &mut workers {
        worker.pass(s.world());
    }
    ...
    for _ in 0..2 {
        for worker in &mut workers {
            assert_eq!(worker.pass(s.world()), Wake::After(Duration::ZERO));
        }
    }
    assert_eq!(*calls.borrow(), [5, 1, 2]);
    let wait = workers.iter_mut().filter_map(|worker| match worker.pass(s.world()) {
...
fn a_failed_store_write_does_not_spin_on_the_same_feed() {
    let s = session();
    s.store().write(|c| c.execute(
        "UPDATE rss_feed SET requested=requested+1 WHERE id=1", [],
    )).unwrap();
    let calls = record(&s);
    let mut workers = RSS.workers(s.store());
    s.store().db().set_writable(false);
    for worker in &mut workers {
        assert!(matches!(worker.pass(s.world()), Wake::After(wait) if wait >= Duration::from_secs(1)));
    }
    assert_eq!(*calls.borrow(), [1]);
}

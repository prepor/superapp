...
impl TerminalBody {
    ...
    fn engine(
        ...
        cx: &mut Cx,
        props: &PanelProps,
        f: impl FnOnce(&mut dyn engine::SurfaceEngine) -> engine::Result<()>,
    ) {
        {
            let mut panel = props.panel.borrow_mut();
            let Some(panel) = panel.as_any().downcast_mut::<TerminalPanel>() else {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        let count = if up {
            ...
            panel
                ...
                .and_then(|panel| panel.engine_found())
...
impl Widget for TerminalView {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        let has_keyboard = keys && (self.bound.is_none() || cx.has_key_focus(self.area));

...
impl TerminalPanel {
    ...
    fn engine_found(&self) -> Option<(usize, usize)> {
        self.shared
            .as_ref()
            .and_then(engine::SurfaceEngine::found)
            .or_else(|| self.engine.as_ref().and_then(engine::Engine::found))
    }
...
impl Panel for TerminalPanel {
    ...
    fn verbs(&self) -> Vec<Verb> {
        ...
        if self.engine.is_some() || self.shared.is_some() {
        ...
        match verb {
            "terminal.restart" => {
                if let Some(shared) = self.shared.take() {
                    shared.close();
                    let replacement =
                        SessionHandle::start(shared.id.clone(), self.mode, shared.cwd);
                    if let Some(registry) = self.registry.as_ref().and_then(Weak::upgrade) {
                        registry
                            .entries
                            .lock()

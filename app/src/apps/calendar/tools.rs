...
pub fn all() -> Vec<Tool> {
 ...
 Tool::new("calendar.availability_result","Read a free/busy request, per-calendar coverage, checked time and candidate slots. Each person includes checks with account addresses and errors; an empty error identifies the successful account. Google error reasons are retained. Unknown is not free; no slot is reserved.",schema(json!({"request":number()}),&["request"]),false,|s,v|{let(q,r,error,_)=availability::load(s.store(),id(v,"request")?).ok_or("request not found")?;Ok(json!({"query":q,"result":r,"error":error}))}),
]
}

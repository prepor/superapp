use super::{availability, edit, model, panels, scoped, sync};
...
pub fn all() -> Vec<Tool> {
 ...
 Tool::new("calendar.events","Read cached occurrences in an explicit time range. start/end are RFC3339 or local ISO datetimes plus an IANA zone. This is not an availability check; inspect cache coverage. Filter supports @calendar, @account, @with, @invited and @meet.",schema(json!({"start":string(),"end":string(),"zone":string(),"filter":string(),"offset":number(),"limit":number()}),&["start","end","zone"]),false,|s,v|{
  ...
  Ok(json!({"events":rows.as_ref(),"total":total,"truncated":offset+rows.len()<total,"coverage":model::sync_line(s.store())}))}),
 ...
 Tool::new("calendar.availability","Queue Google free/busy for participants in an explicit range (up to 14 days) using a connected account. Includes owned calendars even when hidden by filters. Missing sharing permission stays unknown; suggestions may have partial coverage. Query result with calendar.availability_result.",schema(json!({"account":number(),"draft":number(),"start":string(),"end":string(),"zone":string(),"minutes":number(),"guests":{"type":"array","items":string()}}),&["account","start","end","zone","minutes","guests"]),false,|s,v|{let q=availability::Query{start:v["start"].as_str().unwrap_or("").into(),end:v["end"].as_str().unwrap_or("").into(),zone:v["zone"].as_str().unwrap_or("").into(),minutes:v["minutes"].as_u64().unwrap_or(0) as u32,guests:v["guests"].as_array().into_iter().flatten().filter_map(Value::as_str).map(str::to_string).collect()};let id=availability::request(s,id(v,"account")?,q,v["draft"].as_i64())?;reveal(s,panels::Availability::id(id));Ok(json!({"request":id}))}),
 Tool::new("calendar.availability_result","Read a free/busy request, per-calendar coverage, checked time and candidate slots. Unknown is not free; no slot is reserved.",schema(json!({"request":number()}),&["request"]),false,|s,v|{let(q,r,error,_)=availability::load(s.store(),id(v,"request")?).ok_or("request not found")?;Ok(json!({"query":q,"result":r,"error":error}))}),
]
}

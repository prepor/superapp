...
pub fn all() -> Vec<Tool> {
 ...
 Tool::staging("workshop.github.comment","Publish to GitHub directly. path selects a complete published file; snapshot_id pins its reviewed comparison (defaults to the current saved snapshot). Publication never substitutes newer code. Omit path for a general PR comment; create a draft PR first if none exists.",schema(json!({"workspace_id":integer(),"path":{"type":["string","null"]},"text":text(),"snapshot_id":integer()}),&["workspace_id","text"]),true,comment).asking(),

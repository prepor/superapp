...
pub fn project_topic(c: &Connection, t: &IncomingTopic) -> rusqlite::Result<()> {
    ...
    // The count is whichever is larger of the lines cached past the settled
    // cursor and the refresh's own count rebased onto it — exactly what
    // `read_tx` weighs. Rebasing alone would let a refresh older than a line
    // that has since arrived cancel that line out; counting alone would trust
    // a window that holds only part of a long backlog.
    c.execute("UPDATE tg_topic SET name = COALESCE(?3, name), closed = COALESCE(?4, closed),
        hidden = COALESCE(?5, hidden),
        unread = CASE WHEN ?6 IS NULL THEN unread ELSE MAX(
                      (SELECT COUNT(*) FROM tg_message
                       WHERE chat = ?1 AND topic = ?2 AND out = 0 AND service = 0
                         AND id > MAX(COALESCE(?9, 0), COALESCE(tg_topic.last_read, 0))),
...
// this device just made. Advance `last_read` forward only, and take the
// count `mark_read_tx` would — the larger of the lines cached past the
// settled cursor and the snapshot's own count rebased onto it. Both halves
// earn their place: rebasing alone lets a snapshot older than a line that
// has since arrived cancel that line out, and counting alone trusts a
// window that may hold only part of a long backlog. Keeping the local count
// instead would be worse still, since our cursor may sit on one of my own
// outgoing lines, which Telegram's inbox cursor never reaches.
  ...
  unread = MAX(
    (SELECT COUNT(*) FROM tg_message
      WHERE chat = tg_chat.peer AND out = 0 AND service = 0
        AND id > MAX(COALESCE(excluded.last_read, 0), COALESCE(tg_chat.last_read, 0))),

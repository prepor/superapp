...
impl Stage {
    ...
    fn draw_panel_full(
        ...
        // Everything this panel reads while drawing is its provenance: the
        // trace behind the panel context (`cmd+i`). Closed by the caller,
        // since this returns early on a body with no room in it.
    ...
    /// The problems mark: what stands wrong, in the chrome's bottom corner,
    /// in the one colour and static. A toast announced each of them once;
    /// this stays until they clear, and clicking it goes to the panel that
    /// lists them.
    ///
    /// Drawn and registered *before* the overlays' wash and the locked
    /// screen: both own every hit while they are up, so the mark has to sit
    /// under them rather than over. Answers how far it lifts the toasts.
    fn draw_problems_mark(
        &mut self,
        cx: &mut Cx2d,
        sh: &mut super::stage::Shell,
        vp: Rect,
    ) -> f64 {
        let n = sh.session.problems().len();
        if n == 0 {
            return 0.0;
        }
        let msg = kernel::problems::Announced::count_line(n);
        ...
        let hovered = sh.hover.as_ref() == Some(&Act::Problems);
        ...
        self.hits
            .push(Hit::act("problems", r, MouseCursor::Hand, Act::Problems));
        h + 6.0
    }
    ...
    fn draw_sheet(&mut self, cx: &mut Cx2d, sh: &mut super::stage::Shell, vp: Rect) {
        ...
        // a run's pictures are the same every time. The mark keeps the
        // bottom row when one stands — it is a click target and a toast is
        // not — so the stack starts above it.
        let now = sh.session.now();
        sh.toasts.retain(|t| now - t.at <= 3.0);
        let mut y = vp.pos.y + vp.size.y - 12.0 - lift;

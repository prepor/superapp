...
header, the left column two cells joined — rendered for every target:
  ...
  app/resources/icon_{32,64,128,256,512,1024}.png
      makepad's dock icon. `platform/build.rs` bakes into the binary the
      PNGs `MAKEPAD_APP_ICON_*` names — see `.cargo/config.toml`, which is
      how this tree points at them, the files not sitting next to `target/`
      where its own search would look. `cargo-makepad` desktop and android
      builds read the same files.
  app/resources/icon.icns    the bundle icon (`cargo-makepad desktop build`).
  app/resources/icon.ico     the windows executable icon.
  app/resources/android/res/ the launcher icon: an adaptive icon (one vector
    ...
    python3 app/resources/make_icons.py [--preview DIR]
...
def adaptive_foreground_xml() -> str:
    ...
    return (
        ...
        "<!-- generated by app/resources/make_icons.py -->\n"

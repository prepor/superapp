...
//! the replication engine, and the HTTP object transport — with a live
//! `bucketd`-style server in-process. No GUI, no emulator: this is the
//! mechanism the mac and android apps run, driven straight, so it works in a
//! headless environment where the window event loop does not.
//!
...
use superapp::mail;
use superapp::object::{serve_conn, HttpBucket};
...
fn main() {
    ...
    println!("\n✓ full lease lifecycle, over real HTTP: bootstrap, install, sync both ways,");

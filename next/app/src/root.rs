...
//! stage.
//!
//! This is the one file outside `shell/` that names an app, and it is where
//! the CR says the naming belongs: the binary is the only place that knows
//! which apps exist. `AppMain::script_mod` calls the shell's block, then
//! each app's, then its own — and its own is the window, whose stage
//! carries one named child per panel template in the build.
            ...
            main_window := Window{
                ...
                body +: {
                    ...
                    stage := mod.widgets.Stage{
                        ...
                        // Mail's four tags. Both mailboxes draw with one
                        // widget, hung twice: a template is instantiated
                        // per slot, and the two lists are two panels.
...
/// screen from whoever is using the Mac.
#[cfg(all(target_os = "macos", not(headless)))]
fn background_run() -> bool {
    shell::boot::config().e2e.is_some()
}
...
impl App {
    ...
    fn shape(&mut self, cx: &mut Cx) {
        ...
        win.configure_window(cx, size, pos, false, "superapp-next".to_string());
        if !background_run() {
...
/// The hook the binary calls: `app_main!` generates the real `fn main`.
pub fn run() {
    main();
}

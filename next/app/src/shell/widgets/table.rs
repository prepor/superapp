...
impl<S: RowSpec> TableView<S> {
    ...
    fn press(
        ...
        let p = e.abs;
        // The completion's own rows, which are drawn over everything else.
        if let Some(i) = self.picks.iter().position(|r| r.contains(p)) {
            let field = view.text_input(cx, FILTER);
            let ac = &mut self.ac;
    ...
    pub fn draw(
        ...
        if !self.primed {
            self.primed = true;
            if !S::default_filter().is_empty() {
                field.set_text(cx, S::default_filter());
            }
        }
        let text = field.text();
        let focused = field.key_focus(cx);

...
script_mod! {
    ...
    /** One file, as a panel shows it: its name, what it is, when it
        changed, where it lives, and whatever preview there is.

        A panel fills it from a `CardData` it built itself — the disk reads
        are the instance's, so the card draws the same whether its bytes
        came off a disk or out of a letter. */

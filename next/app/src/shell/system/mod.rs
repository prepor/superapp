...
script_mod! {
    ...
    mod.widgets.SysAboutPanel = set_type_default() do #(AboutPanel::register_widget(vm)) {
        ...
        mod.widgets.SSection { text: "SUPERAPP — NEXT" }
        mod.widgets.SRule {}
        mod.widgets.SRow {
            mod.widgets.SLabel { width: Fill, text: "the prototype of CR-010: a kernel that does not draw, a shell that does, and apps on top of both." }
        }
        mod.widgets.SRow {

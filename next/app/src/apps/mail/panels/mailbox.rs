...
impl Mailbox {
    ...
    /// The slot it landed in — what its bar's links navigate from.
    #[must_use]
    pub fn slot(&self) -> SlotId {
        self.slot
    }

    /// The table, its cursor and its marks: what the table widget draws and
    /// changes.
    #[must_use]
    ...
    /// The store it was opened with — a widget reads its rows through this.
    #[must_use]
    ...
    /// The draw's own step: what the filter shows of the marks and what it
    /// hides, read fresh.
    ...
    /// Hands the filter field's text to the table. Answers whether it
    /// changed.
    ...
    #[must_use]
    pub fn cursor_thread(&self) -> Option<i64> {
        self.list.cursor_key().copied()
    }
    ...
    /// Enter: the row under the cursor, opened rather than previewed.
    pub fn open_at(&mut self, i: usize) -> Option<Nav> {
        let store = self.store.clone();
        let row = self.list.set_cursor(&store, i)?;
        Some(Nav::Open {
            from: self.slot,
            id: Message::id(row.target),
            fresh: false,
        })
    }
...
impl Panel for Mailbox {
    ...
    /// count, and the two verbs about the set itself. Nothing archives out of
    /// the archive, so that half of the bar belongs to the inbox alone.
    ///
    /// *mark all* wears `m` rather than the `l` of the shipping app: this
    /// shell keeps `cmd+l` for itself (see
    /// [`keys`](crate::shell::keys)), and a bar may not promise a chord that
    /// never arrives. *clear* wears none — `esc` is the table's own.
...
impl PanelKind for MailboxKind {
    ...
    fn open(&self, id: &PanelId, cx: &mut Opening<'_>) -> Box<dyn Panel> {
        Box::new(Mailbox {
            ...
            slot: 0,
            list: ListState::new(model::threads(self.0), MAILBOX_PAGE),
        })
    }
}

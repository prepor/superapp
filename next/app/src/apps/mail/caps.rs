//! The two servers mail talks to, and the fake that stands in for both.
//!
//! An app defines its own capabilities and supplies them in
//! [`App::outside`](kernel::app::App::outside). Mail's are [`Imap`] and
//! [`Smtp`]; the prototype has neither a real IMAP nor a real SMTP, so
//! [`FakeServers`] is installed under every mode. It registers itself under
//! both traits *and* its concrete type, so a test can reach
...
pub trait Imap {
    ...
    /// The uids in the folder: every one, or those without `\Seen`.
    ///
    /// # Errors
    ///
    /// If there is no such folder.
...
/// What every world's mail capability is in the prototype: one in-memory
/// server per account, a keychain that is a map, and a switch that takes the
/// whole thing offline.
///
...
impl FakeServers {
    ...
    pub fn demo() -> FakeServers {
        let s = FakeServers::new();
        {
            ...
            for m in seed::mails() {
                server.deliver(seed::folder_name(m.folder), m.unread, &seed::rfc822(&m));
            }
    ...
    /// Plants a password, as a settings form would.
    pub fn keychain(&self, email: &str, pass: &str) {
        if let Ok(mut g) = self.0.lock() {
            g.secrets.insert(email.to_string(), pass.to_string());
        }
    }
...
impl Smtp for FakeServers {
    fn submit(&mut self, c: &Creds, m: &Outgoing) -> Result<Vec<u8>, String> {
        ...
        g.by_account.entry(acct).or_default().submitted.push(m.clone());
        Ok(raw.into_bytes())
    }
}
...
///
/// The fake goes in under every mode: the prototype has no real IMAP and no
/// real SMTP, and a `Deny` world that answered *this world has no Imap* would
/// be telling the truth about a build that does not exist. The password the
/// demo account signs in with is planted in the world's own keychain, so
/// [`creds_for`](super::model::creds_for) finds it exactly as it would find a
/// human's.
pub fn install(_mode: Mode, env: &Env, caps: &mut Capabilities) {

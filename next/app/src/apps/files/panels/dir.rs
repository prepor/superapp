...
impl Dir {
    ...
    /// Where this panel stands.
    #[must_use]
    pub fn dir(&self) -> &str {
        &self.dir
    }

    /// The table, the cursor and the marks — what the table widget draws
    /// and changes.
    #[must_use]
    ...
    /// What `enter` sends: the row opened beside the list and focused, or
    /// — with cmd — in a panel of its own.
    #[must_use]
    pub fn open(&self, e: &Entry, fresh: bool) -> Nav {
        Nav::Open {
            from: self.slot,
            id: self.row_id(e),
            fresh,
...
impl Panel for Dir {
    ...
    /// wears the same letter; and while the clipboard holds something,
    /// `copy here` or `move here` names what will happen to it.

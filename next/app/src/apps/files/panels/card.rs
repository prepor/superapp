...
use super::super::model::{
    basename, fmt_size, read_in, stat_in, text_lines, Entry, FileKind, TEXT_PREVIEW_MAX,
};
use super::super::ops;
...
/// whenever a verb writes the disk: the entry, and — for a text file — the
/// first 64 KiB of it.
pub struct Card {
    id: PanelId,
    path: String,
...
impl Card {
    ...
    pub fn kind_line(&self) -> String {
        format!("{} · {}", self.kind_word(), fmt_size(self.size()))
    }
    ...
    /// The preview: the first 64 KiB of a text file, or nothing at all.
    /// The prototype draws no pictures.
    #[must_use]
    ...
    /// Reads the file again: what it is, and what it says.
    pub fn restat(&mut self) {
        self.listed = FILES.writes();
        self.entry = stat_in(&self.world, &self.path);
        self.text = self.entry.as_ref().and_then(|e| {
            (e.kind() == FileKind::Text)
                .then(|| read_in(&self.world, &self.path, TEXT_PREVIEW_MAX).ok())
                .flatten()
...
impl Panel for Card {
    ...
    /// text file opens tall rather than scrolled — up to what a grid is
    /// likely to hold. The layout clamps it to the grid there actually is.
    fn wish(&self, cols: usize) -> (u32, u32) {
        let lines = self.text.as_deref().map_or(0, |t| text_lines(t, cols));

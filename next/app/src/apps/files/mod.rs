...
// Files is written against its own interfaces, not against this build's use
// of them: the model answers questions no panel asks yet — how many paths
// the clipboard holds, the listing behind a filter, a card's kind and size
// on one line. Told once here rather than one `pub` at a time.
#![allow(dead_code)]

...
//! that app's [`Schema`](crate::app::Schema), applied after the kernel's ladder
//! in app-list order.
...
pub struct Db {
    ...
    target: Target,
    dir: Option<PathBuf>,
    /// Whether ordinary [`Db::write`] mutations are allowed. Device sync's
    /// follower holds this `false` in the shipping tree; the prototype
    /// leaves it open, and keeps the gate so the verbs that ask are written
...
impl Db {
    ...
    /// Opens the gate to ordinary writes, or closes it.
    ...
    fn write<T: Send + 'static>(
        ...
        self.jobs.send(Job { run, reply }).map_err(|_| gone())?;
...
/// This install's device id — the one `meta` holds. Empty when the row is
...
pub fn this_device(conn: &Connection) -> String {
    conn.query_row("SELECT value FROM meta WHERE key = 'device'", [], |r| {
        r.get(0)
    })
    .unwrap_or_default()
}

/// A stable per-install device id: two devices must never share one. No
/// `rand` dependency — a mix of the wall clock, the pid and a process-local
/// counter is unique enough for two devices.
fn device_id() -> String {
...
fn writer_loop(conn: &Connection, dirty: &Arc<Mutex<HashSet<String>>>, rx: &mpsc::Receiver<Job>) {
    while let Ok(Job { run, reply }) = rx.recv() {
        let _ = reply.send(do_write(conn, dirty, run));
    }
}

/// One write, on the writer thread: begin, run the closure, commit. A panic
/// or error rolls the whole thing back.
fn do_write(conn: &Connection, dirty: &Arc<Mutex<HashSet<String>>>, run: RunFn) -> WriteOut {
...
impl Store {
    ...
    /// Opens or closes the write gate.
    pub fn set_writable(&self, writable: bool) {

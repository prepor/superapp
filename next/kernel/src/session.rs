...
impl Session {
    ...
    /// the workers. Refuses with a toast when not writable. Returns what
    /// `data` returned, which is how an action learns a new row id.
    ///
...
mod tests {
    ...
    fn a_relayout_asks_every_instance_what_it_wants() {
        ...
        s.nav(crate::nav::Nav::Close(slot));
        s.settle();
    ...
    fn a_closed_gate_refuses_an_action() {
        ...
        // `act_done` skips the session's gate — the caller has already
        // touched the disk — but the store's own is still shut, so it fails
        // there instead, in the store's words.
        assert!(s
        ...
        // With the lease back, the same action lands.

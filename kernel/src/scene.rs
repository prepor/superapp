...
//! The shell supplies real fixtures; this module only stores and lays out
//! the graph and does not depend on Makepad.
...
mod tests {
    ...
    fn check_rejects_bad_shapes() {
        ...
        let nowhere: Scene<()> = Scene::new("s", (1.0, 1.0)).node("a", ()).edge("a", "b", "");
        assert!(nowhere.check().unwrap_err().contains("names no node"));
        let selfloop: Scene<()> = Scene::new("s", (1.0, 1.0)).node("a", ()).edge("a", "a", "");

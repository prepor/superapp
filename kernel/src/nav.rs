...
pub enum Nav {
    ...
    /// only the caller can read: `nav` may not touch an instance, since a
    /// verb that closes its own panel is running as `&mut self` on it. A
    /// caller with nothing to say leaves it `None` and the node names the
    /// identity instead.
    Close {
        slot: SlotId,
        label: Option<String>,
...
impl Session {
    ...
    pub fn nav(&mut self, n: Nav) {
        match n {
            ...
            Nav::Close { slot, label } => {
                let what = label.unwrap_or_else(|| self.shows(slot));
                self.act(
                    Action::new("close", format!("close “{what}”"))
    ...
    /// What a slot shows, as an action labels it when the caller offered no
    /// title of its own.
    ///
...
mod tests {
    ...
    fn close_takes_the_chain_and_records_a_node() {
        ...
        assert_eq!(s.panels().len(), 2);
        go(&mut s, Nav::Close { slot: list_slot, label: None });
    ...
    fn a_nav_names_its_slot_and_its_open() {
        ...
        assert_eq!(Nav::Close { slot: 3, label: None }.how(), None);
        assert_eq!(Nav::Focus(3).how(), None);
        assert_eq!(Nav::Close { slot: 7, label: None }.from(), 7);
        assert_eq!(
            Nav::Preview {
                from: 7,
                id: list()

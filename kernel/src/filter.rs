...
pub fn typing_tag(input: &str) -> Option<&str> {
    ...
    partial.chars().all(is_tag_char).then_some(partial)
}
...
mod tests {
    ...
    fn negation() {
        assert_eq!(
            ast("@not:deleted"),
            Some(Ast::Not(Box::new(tag("deleted"))))
        );
        assert_eq!(
            ast("@not:role:guest"),
    ...
    fn or_groups() {
        ...
        assert_eq!(ast("(@a @b)"), Some(Ast::Or(vec![tag("a"), tag("b")])));
    }
    ...
    fn splicing() {
        assert_eq!(
            insert_tag("vera @un", 8, 5, "unread", false),
            ("vera @unread ".into(), 13)
        );
        assert_eq!(
            insert_tag("@fr x", 3, 0, "from", true),
            ("@from: x".into(), 6)
        );
        assert_eq!(

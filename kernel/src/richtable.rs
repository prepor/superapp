...
pub trait Datasource {
    ...
    /// A key as text — how one travels where a type cannot: an argument of
    /// a [`PanelId`](crate::panel::PanelId), a launcher hit, an intent's
    /// record of what a batch verb consumed. The spelling is the source's,
    /// and it is stable: a key written down is read back by the same source
    /// later.
    fn key_text(&self, key: &Self::Key) -> String;

    /// The key back. `None` for text this source cannot read.
    fn key_parse(&self, text: &str) -> Option<Self::Key>;
...
impl SqlSpec {
    ...
    fn body_and(
        ...
        if ast
            .and_then(|a| self.expr(tags, a, &mut Vec::new()))
            .is_some()
        {
        ...
        + 'static,
{
    ...
    /// A SQL key is a number or a string, and both write themselves.
    fn key_text(&self, key: &K) -> String {
        key.to_string()
    }

    fn key_parse(&self, text: &str) -> Option<K> {
        text.parse().ok()
    }
...
impl<D: Datasource> Table<D> {
    ...
    pub fn len(&self, store: &Store) -> usize {
        ...
        for p in 0..self.window {
            let page = self
                .ds
                .page(store, self.ast.as_ref(), p * self.page_size, self.page_size);
            n += page.len();
...
/// What one list item is: a marked row the filter hides, the caption or the
/// rule around them, or a row of the table.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MarkSlot<R> {
    /// *3 marked, hidden by the filter* — the line above the hidden rows.
    Caption,
    /// One of them.
    Hidden(R),
...
/// The whole state a panel that shows a list owns: the [`Table`], the
/// cursor, and the [`Marks`].
///
/// This is the kernel half of the rich table. The shell's table widget
/// borrows one from the scope every draw and event and answers a press by
/// its own row geometry; everything it changes, it changes here. Three
...
impl<D: Datasource> ListState<D> {
    ...
    #[must_use]
    pub fn table(&self) -> &Table<D> {
        &self.table
    }

    pub fn table_mut(&mut self) -> &mut Table<D> {
        &mut self.table
    }

    #[must_use]
    pub fn marks(&self) -> &Marks<D::Key> {
        &self.marks
    }
    ...
    /// Points the list at another source of the same shape — a panel
    ...
    /// changed. Answers whether it did.
    pub fn set_filter(&mut self, text: &str) -> bool {
    ...
    pub fn move_cursor(&mut self, store: &Store, d: isize) -> Option<D::Row> {
        ...
        self.set_cursor(store, i)
    }

    /// Forgets where the cursor was — what a panel does when its source
    /// changed under it.
    ...
    /// fresh panel, a filter just cleared — the top row is the row, since
    ...
    /// Shift+arrow: marks the row the cursor stands on, steps, and marks
    /// the row it lands on — a range, by the keys the walk already uses.
    /// Answers the row it landed on, which is what a preview follows.
    pub fn mark_range(&mut self, store: &Store, d: isize) -> Option<D::Row> {
        self.mark_cursor(store);
        let landed = self.move_cursor(store, d);
        self.mark_cursor(store);
        landed
    }
    ...
    /// How many list items ride above the table's own rows: the caption,
    /// the hidden marks, and the rule under them — none at all when the
    ...
    /// The list index of table row `i` — where a scroll-follow has to look
    /// for it.
    #[must_use]
...
mod tests {
    ...
    fn builds_the_marks_queries() {
        ...
        assert_eq!(
            q.sql,
            "SELECT id FROM item WHERE id > 0 ORDER BY n DESC, id"
        );
        ...
        assert_eq!(
            q.sql,
            "SELECT id FROM item WHERE id > 0 AND id IN (?, ?, ?)"
        );
        ...
        assert_eq!(
            q.sql,
            "SELECT id, name, n, ok, at FROM item WHERE id > 0 AND id = ?"
        );
    ...
    fn builds_every_filter_shape() {
        ...
        assert_eq!(
            w("@not:ok"),
            (
                " WHERE id > 0 AND NOT COALESCE(((ok = 1)), 0)".into(),
                vec![]
            )
        );
        assert_eq!(
        ...
        assert_eq!(
            w("@n>3"),
            (" WHERE id > 0 AND n > ?".into(), vec![Val::F(3.0)])
        );
        assert_eq!(
            w("@n<=3.5"),
            (" WHERE id > 0 AND n <= ?".into(), vec![Val::F(3.5)])
        );
        assert_eq!(
            w("@n:x"),
            (" WHERE id > 0".into(), vec![]),
        ...
        assert_eq!(
            w("@at>30.08.2026"),
            (
                " WHERE id > 0 AND at >= ?".into(),
                vec![Val::F(d0 + 86_400.0)]
            )
        );
        assert_eq!(
            w("@at>=30.08.2026"),
            (" WHERE id > 0 AND at >= ?".into(), vec![Val::F(d0)])
        );
        assert_eq!(
            w("@at<30.08.2026"),
            (" WHERE id > 0 AND at < ?".into(), vec![Val::F(d0)])
        ...
        assert_eq!(
            ...
            (
                " WHERE id > 0 AND (at >= ? AND at < ?)".into(),
                vec![
                    Val::F(d0 + 9.0 * 3600.0 + 14.0 * 60.0),
                    Val::F(d0 + 9.0 * 3600.0 + 15.0 * 60.0)
                ]
            )
        );
    ...
    fn a_table_pages_through_the_store() {
        ...
        assert_eq!(
            t.rows(&s, 0, 9).iter().map(|i| i.id).collect::<Vec<_>>(),
            vec![7, 21]
        );
        let d = days_from_civil(2026, 8, 1) + 20;
        t.set_filter(&format!(
            "@at>{}.08.2026",
            d - days_from_civil(2026, 8, 1) + 1
        ));
        assert_eq!(t.len(&s), 5);
        ...
        assert_eq!(
            t.errors()[0].message,
            "@at wants a day, dd.mm.yyyy: yesterday"
        );
        ...
        s.write(|c| {
            c.execute(
                "INSERT INTO item(name, n, ok, at) VALUES('new', 9, 1, 0)",
                [],
            )
        })
        .unwrap();
    ...
    fn marks_are_a_set_of_keys() {
        ...
        assert_eq!(
            m.keys(),
            vec![1, 2, 3],
            "in key order, whatever the order marked"
        );
    ...
    fn suggestions_for_tags_and_values() {
        ...
        assert_eq!(
            (sug[0].label.as_str(), sug[0].value.as_str()),
            ("today", "02.09.2026")
        );
        assert_eq!(sug[0].describe, "02.09.2026");
        assert_eq!(
            names(t.suggestions(&s, &ctx("@at:01.09"))),
            vec!["yesterday"]
        );
        ...
        assert_eq!(
            names(t.suggestions(&s, &ctx("@name:\"item 002"))),
            vec!["item 002 gamma"]
        );
    ...
    fn the_filter_is_a_completion() {
        ...
        assert_eq!(
            pick("@ok @name:\"item 00", 0),
            ("@ok @name:\"item 001 beta\" ".into(), 26)
        );
        assert_eq!(pick("x @at:t", 0), ("x @at:02.09.2026 ".into(), 17));
        assert_eq!(t.context("plain words", 5), None);
    }

    /// A key travels as text and comes back, which is how one lands in a
    /// panel identity's arguments.
    #[test]
    fn keys_travel_as_text() {
        let s = &SOURCE;
        assert_eq!(s.key_text(&7), "7");
        assert_eq!(s.key_parse("7"), Some(7));
        assert_eq!(s.key_parse("seven"), None, "what it cannot read");
    }
    ...
    fn the_cursor_holds_its_row_then_its_key_then_its_place() {
        ...
        assert_eq!(l.cursor_index(&s), None, "a fresh list has no cursor");
        assert_eq!(l.cursor_key(), None);

        // An arrow with no cursor lands on the top row.
        let row = l.move_cursor(&s, 1).expect("the top row");
        assert_eq!(row.id, 6);
        assert_eq!(l.cursor_index(&s), Some(0));
        assert_eq!(l.cursor_key(), Some(&6));

        l.move_cursor(&s, 3);
        assert_eq!(l.cursor_index(&s), Some(3));
        let held = *l.cursor_key().unwrap();

        // Something lands above it: the index moved, the row did not.
        s.write(|c| {
            c.execute(
                "INSERT INTO item(name, n, ok, at) VALUES('new', 9, 1, 0)",
                [],
        ...
        l.set_cursor(&s, 20);
        s.write(|c| c.execute("DELETE FROM item WHERE id > 5", []))
            .unwrap();
        let n = l.len(&s);
        assert_eq!(l.cursor_index(&s), Some(n - 1));

        // An empty list has no cursor to stand anywhere. (`WHERE` on
        // purpose: a bare `DELETE FROM` takes SQLite.s truncate path and
        // fires no update hook, so nothing would go stale.)
        s.write(|c| c.execute("DELETE FROM item WHERE id > 0", []))
            .unwrap();
        assert_eq!(l.cursor_index(&s), None);

        // A new filter puts it back at nothing.
        let s = store_with(5);
        let mut l = ListState::new(&SOURCE, 10);
        l.move_cursor(&s, 1);
    ...
    /// walks a range, `all` takes the filter's whole set, and a mark the
    /// filter hides rides above the rows.
    #[test]
    fn a_list_marks_toggles_ranges_and_all() {
        let s = store_with(25);
        let mut l = ListState::new(&SOURCE, 10);

        // Space with no cursor marks the top row.
        assert!(l.toggle_mark(&s));
        assert_eq!(l.marks().keys(), vec![6]);
        assert!(l.toggle_mark(&s));
        assert!(l.marks().is_empty(), "and unmarks it again");

        // A range: two rows, by the keys the walk uses.
        l.set_cursor(&s, 0);
        l.mark_range(&s, 1);
        assert_eq!(l.marks().len(), 2);
        assert_eq!(l.cursor_index(&s), Some(1));

        // All of it, the rows off screen included.
        l.clear_marks();
        assert!(l.mark_all(&s));
        assert_eq!(l.marks().len(), 25);

        // Under a filter, `all` is the filter's set — and the rest of what
        // was marked is hidden, not lost.
        l.clear_marks();
        l.marks_mut().extend([6, 7, 20]);
        l.set_filter("@ok"); // 7 is odd
        assert_eq!(l.split(&s), (vec![6, 20], vec![7]));
        l.sync(&s);
        assert_eq!(l.hidden_rows().len(), 1);
        assert_eq!(l.hidden_rows()[0].id, 7);
        assert_eq!(l.marks().len(), 3, "a hidden mark is still a mark");

        // The list the widget draws: caption, the hidden rows, a rule,
        // then the table's own.
        assert_eq!(l.prefix(), 3);
        assert_eq!(l.list_index(0), 3);
        assert_eq!(l.slot(0), MarkSlot::Caption);
        assert_eq!(l.slot(1), MarkSlot::Hidden(l.hidden_rows()[0].clone()));
        assert_eq!(l.slot(2), MarkSlot::Rule);
        assert_eq!(l.slot(3), MarkSlot::Row(0));

        // A mark whose row is gone goes with it.
        s.write(|c| c.execute("DELETE FROM item WHERE id = 7", []))
            .unwrap();
        l.sync(&s);
        assert_eq!(l.marks().keys(), vec![6, 20]);
        assert_eq!(l.prefix(), 0, "nothing hidden, nothing above the rows");
        assert_eq!(l.slot(1), MarkSlot::Row(1));
    ...
    fn a_countless_list_offers_no_mark_all() {
        ...
        l.sync(&s);
        assert_eq!(l.marks().len(), 2);
        assert_eq!(l.hidden_rows().len(), 0, "nothing is known to be hidden");
        assert_eq!(l.len(&s), 10);
        assert_eq!(l.row(&s, 0), Some(0));
    }

    /// Retargeting a list is a new source and a clean slate.
    #[test]
    fn retargeting_drops_the_cursor_and_the_marks() {
        let s = store_with(9);
        let mut l = ListState::new(&SOURCE, 10);
        l.move_cursor(&s, 1);
        l.toggle_mark(&s);
        l.sync(&s);
        assert!(l.cursor_key().is_some() && !l.marks().is_empty());
        l.retarget(&SOURCE);
        assert_eq!(l.cursor_key(), None);
        assert!(l.marks().is_empty() && l.hidden_rows().is_empty());
    }
